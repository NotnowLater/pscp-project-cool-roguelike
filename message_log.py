...
class MessageLog:
    ...
    def wrap_text(text : str, width : int) -> Iterable[str]:
        """ Return a wrapped text message of given width. """
        for line in text.splitlines():
            yield from textwrap.wrap(line, width=width, expand_tabs=True)
    ...
    def render_messages(
        ...
        for message in reversed(messages):
            for line in reversed(list(cls.wrap_text(message.full_text, width))):
